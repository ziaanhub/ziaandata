...
ZiaanUI.Themes = {
    ...
    Purple = {
        ...
        Section = Color3.fromRGB(45, 40, 65),
        Text = Color3.fromRGB(255, 255, 255),
        SubText = Color3.fromRGB(200, 180, 255),
        Border = Color3.fromRGB(65, 60, 85)
    }
}

-- Create new ZiaanUI
function ZiaanUI.new(options)
    options = options or {}
    
    local self = setmetatable({}, ZiaanUI)
    
    self.Config = {
        Theme = options.Theme or ZiaanUI.Config.Theme,
        AccentColor = options.AccentColor or ZiaanUI.Config.AccentColor,
        Font = options.Font or ZiaanUI.Config.Font,
        AnimationSpeed = options.AnimationSpeed or ZiaanUI.Config.AnimationSpeed,
        Rounding = options.Rounding or ZiaanUI.Config.Rounding,
        BlurBackground = options.BlurBackground or ZiaanUI.Config.BlurBackground,
        Name = options.Name or "ZiaanUI Hub",
...
-- Create main UI
function ZiaanUI:CreateUI()
    -- ScreenGui
    self.ScreenGui = Instance.new("ScreenGui")
    self.ScreenGui.Name = "ZiaanUI_" .. self.Config.Name
...
function ZiaanUI:Draggable(frame)
...
function ZiaanUI:Toggle()
    self.Visible = not self.Visible
    self.MainFrame.Visible = self.Visible
    if self.Blur then
        self.Blur.Enabled = self.Visible
    end
end

-- Show UI
function ZiaanUI:Show()
    self.Visible = true
...
-- Add tab
function ZiaanUI:AddTab(name, icon)
    local tab = {}
    tab.Name = name
    tab.Icon = icon
    ...
    tab.Button.Position = UDim2.new(0, 5, 0, (#self.Tabs * 40))
    tab.Button.BackgroundColor3 = self.Themes[self.Config.Theme].Border
    ...
    tabCorner.CornerRadius = UDim.new(0, self.Config.Rounding - 2)
        ...
        if self.ActiveTab ~= tab then
            TweenService:Create(tab.Button, TweenInfo.new(0.2), {
                BackgroundColor3 = Color3.fromRGB(
                    self.Themes[self.Config.Theme].Border.R * 1.2,
                    self.Themes[self.Config.Theme].Border.G * 1.2,
                    self.Themes[self.Config.Theme].Border.B * 1.2
                )
            }):Play()
        end
    end)
...
-- Switch to tab
function ZiaanUI:SwitchTab(tab)
    if self.ActiveTab then
        self.ActiveTab.Button.TextColor3 = self.Themes[self.Config.Theme].SubText
        self.ActiveTab.Button.BackgroundColor3 = self.Themes[self.Config.Theme].Border
    ...
    TweenService:Create(tab.Button, TweenInfo.new(0.2), {
        BackgroundColor3 = Color3.fromRGB(
            self.Config.AccentColor.R * 0.8,
            self.Config.AccentColor.G * 0.8,
            self.Config.AccentColor.B * 0.8
        )
    }):Play()
...
-- Add section to tab
function ZiaanUI:AddSection(tab, name)
    local section = {}
    section.Name = name
    ...
    local function updateSize()
        local totalHeight = 40 -- Title height + padding
        for _, element in pairs(section.Elements) do
            totalHeight += element.Height + 8 -- Element height + padding
        end
...
function ZiaanUI:AddButton(section, text, callback)
    local button = {}
    button.Text = text
    ...
    button.Label.Size = UDim2.new(1, -20, 1, 0)
    button.Label.Position = UDim2.new(0, 10, 0, 0)
...
function ZiaanUI:AddToggle(section, text, default, callback)
    local toggle = {}
    toggle.Text = text
...
function ZiaanUI.Toggle:SetValue(value)
    self.Value = value
    if value then
...
function ZiaanUI:AddSlider(section, text, min, max, default, callback)
    local slider = {}
    slider.Text = text
    slider.Min = min or 0
...
function ZiaanUI.Slider:SetValue(value)
    self.Value = value
...
function ZiaanUI:AddDropdown(section, text, options, default, callback)
    local dropdown = {}
    dropdown.Text = text
...
function ZiaanUI:AddLabel(section, text)
    local label = {}
    label.Text = text
    label.Height = 20
...
function ZiaanUI:AddKeybind(section, text, default, callback)
    local keybind = {}
    keybind.Text = text
...
-- Change theme
function ZiaanUI:SetTheme(themeName)
...
-- Change accent color
function ZiaanUI:SetAccentColor(color)
    self.Config.AccentColor = color
    -- You can add logic to update existing elements with new accent color
end

-- Destroy UI
function ZiaanUI:Destroy()
    if self.Blur then
        self.Blur:Destroy()
    end
